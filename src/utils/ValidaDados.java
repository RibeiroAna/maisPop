...
public class ValidaDados {
	...
	public static void validaNome(String nome) {
		if (nome.trim().length() == 0) {
			throw new BadFormatException(MensagensDeErro.CADASTRO_NOME_VAZIO);
		}
	}
	...
	public static void validaData(String data) {
		if (data.split("/").length != 3) {
			throw new BadFormatException(
					MensagensDeErro.CADASTRO_FORMATO_DATA_INVALIDA);
		}
		...
		if (((dia < 0) || (dia > 31) || (mes < 0) || (mes > 12) || (ano < 0) || (ano > 3000))) {
			throw new BadFormatException(
					MensagensDeErro.CADASTRO_FORMATO_DATA_INEXISTENTE);
		}
	}
	...
	public static void validaEmail(String email) {
		  Pattern pattern = Pattern.compile("^[\\w-]+(\\.[\\w-]+)*@([\\w-]+\\.)+[a-zA-Z]{2,7}$"); 
		    ...
		    if (!matcher.find()) {
		    	throw new BadFormatException(MensagensDeErro.CADASTRO_FORMATO_EMAIL_ERRADO);
		    }
	}
